...
def prepare_data(cn_path: Path, en_path: Path) -> pd.DataFrame:
    ...
    for frame in (cn, en):
        ...
        frame["Classification_Score"] = pd.to_numeric(
            frame["Classification_Score"], errors="coerce"
        )
        frame["Classification_Margin"] = pd.to_numeric(
            frame["Classification_Margin"], errors="coerce"
        )
    ...
    # Final_Field may contain the explicit rejection label used by the English
    # classifier. These rows are excluded from final-scope analyses below.
    allowed_labels = {
        "Final_Field": set(FIELDS) | {UNCLASSIFIED},
        "Raw_Field": set(FIELDS),
    }
...
def main() -> None:
    ...
    final_data = data[data["Final_Field"].isin(FIELDS)].copy()
    raw_data = data[data["Raw_Field"].isin(FIELDS)].copy()
    high_confidence = raw_data[raw_data["High_Confidence"]].copy()

    specifications: list[tuple[str, pd.DataFrame, str]] = [
        ("Final classification", final_data, "Final_Field"),
        ("Raw classification", raw_data, "Raw_Field"),
        (
        ...
        (
            ...
            final_data[final_data["Year"] <= 2024].copy(),
            "Final_Field",
        ),
    ]

    excluded = len(data) - len(final_data)
    print(f">> Final classification excludes {excluded:,} Unclassified record(s).")
