...
def load_and_preprocess_custom_data(file_pattern="CN*-*.txt"):
    ...
    df = df[(df['Abstract'].str.strip() != '') & (df['Title'].str.strip() != '') & (df['Year'].str.strip() != '')]
    ...
    clean_df = pd.DataFrame({
        ...
        'Year': df['Year'],
        'Author': df['Author'].str.strip(),
        'Keywords': df['Keywords'].str.strip(),
        'Journal': df['Journal'].str.strip(),
        'Citations': df['Citations']
    })
    print(f">> Retained {len(clean_df)} valid records (2020-2025).")
    return clean_df
...
def filter_chinese_social_science(df):
    ...
    print(f">> Total remaining articles after filtering: {len(df_final)}")
    return df_final
...
def build_chinese_phrases(df, stopwords, min_freq=2, llr_threshold=0.1):
    ...
    for p in all_phrases:
        ...
        if all(part in stopwords for part in parts):
            continue
        if any(word in forbidden_words for word in parts):
            continue
...
def generate_output(word_counter, phrase_counter, df_clean,
    ...
    wc_freq = {k.replace('_', ' '): v for k, v in word_counter.items() if k not in STOPWORDS_CN}
    wc_freq = {k: v for k, v in wc_freq.items() if v >= 3}
    if not wc_freq:
...
if __name__ == "__main__":
    ...
    for p in phrase_list:
        jieba.add_word(p.replace('_', ''), freq=1000)
